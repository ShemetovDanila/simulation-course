...
2. **Распределение состояний системы.** Построенная гистограмма распределения вероятностей наглядно отражает специфику СМО вида $M/M/N/M$. Зеленые столбцы ($P_0$–$P_5$) демонстрируют вероятности состояний, когда система имеет свободные каналы обслуживания, а коралловые столбцы ($P_6$–$P_{15}$) фиксируют состояния ожидания в очереди. Наиболее вероятным является нахождение в системе 2–3 заявок, в то время как крайние состояния загрузки накопителя ($P_{10}$–$P_{15}$) стремятся к нулю, обеспечивая стабильность процесса обработки.